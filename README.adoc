...
http://arquillian.org/[Arquillian] is a testing platform to develop automated functional, integration and acceptance tests for your java applications. Arquillian tests are very similar to the common unit tests, however, Arquillian sets up the test environment and able to handle the application server life cycle for you, so you can focus on writing tests. An Arquillian test has three components, `@RunWith(Arquillian.class)`, `@Deployment` and `@Test`. Once you have these three components setup, you'll have everything you need to test your microservices. You will learn more about these components in the test class.
...
Notice the `arquillian-managed.war` WAR name in the web archive, this is necessary, if you don't want a randomly generated web archive name. This WAR name is also needed when the `inventory` service communicates to the `system` service to get the system properties.
