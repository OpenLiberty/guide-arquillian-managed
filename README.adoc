...
To try out the finished Arquillian tests, first navigate to the `finish` directory and then run
...
You need the [hotspot=liberty-maven-app-parent]`liberty-maven-app-parent` block,
...
You also need the [hotspot=arquillian-bom]`arquillian-bom` Bill of Materials.
...
the [hotspot=configure-arquillian]`configure-arquillian` goal to configure your Arquillian container.
...
You can learn more about the [hotspot=configure-arquillian]`configure-arquillian` goal in its
https://github.com/WASdev/ci.maven/blob/master/docs/configure-arquillian.md[documentation^].

Notice in the `liberty-maven-plugin` configuration that the [hotspot=skipTestServer]`skipTestServer` element is set to
`true` to skip the `test-start-server`
and `test-stop-server` goals defined in the [hotspot=liberty-maven-app-parent]`liberty-maven-app-parent` parent configuration.
