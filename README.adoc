...
You will work with an `inventory` microservices, which stores information about various systems.
...
The `finish` directory in the root of this guide contains the finished Arquillian tests.
Give it a try before you proceed.

To try out the application, first navigate to the `finish` directory and then run
the following Maven goal to build the application and run the tests on Open Liberty:
...
In the console output, you can see the build was successful and the tests passed.
...
The [hotspot=29]`@RunWith` annotation preceding the class tells JUnit to run the tests by using Arquillian.
...
Notice the [hotspot=32 hotspot=38]`arquillian-managed.war` name that is used for the web
...
[hotspot=36-41]`@Deployment` annotated method.
...
The injected [hotspot=46-47]`InventoryResource` instance is then tested
by the [hotspot=86-97]`testInventoryResourceFunctions` method.
...
The [hotspot=140-173]`maven-failsafe-plugin` artifact runs your Arquillian integration tests by using JUnit.
...
Notice in the `liberty-maven-plugin` configuration that the [hotspot=194]`skipTestServer` element is set to
