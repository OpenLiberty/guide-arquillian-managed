...
http://arquillian.org/[Arquillian] is a testing platform to develop automated functional, integration and acceptance tests for your Java applications. Arquillian tests are very similar to the common unit tests, however, Arquillian sets up the test environment and is able to handle the application server life cycle for you, so you can focus on writing tests. An Arquillian test has three components, `@RunWith(Arquillian.class)`, `@Deployment` and `@Test`. Once you have these three components setup, you'll have everything you need to test your microservices. You will learn more about these components in the test class.

Arquillian Liberty Managed container is an adaptor that implements the Arquillian's `DeployableContainer` to run Arquillian tests with Liberty. It's capable of managing the local Liberty server operations, packaging the tests as a `WAR` to be deployed to the Liberty server and then running the tests on the server.

The application that you will work with is an `inventory` service that stores information about various systems. The `inventory` service communicates with the `system` service on a particular host to retrieve its system properties and stores them. You will learn how to configure the `inventory` service to use the Liberty Arquillian Managed container adaptor with the Maven build tool and develop functional and integration tests for the microservices to run in Open Liberty.
...
You will need to configure your build tool to use the Arquillina Liberty Managed container and setup the Liberty `server.xml` file before developing Arquillian test cases for your microservices and running them with Open Liberty.
...
Next, the `arquillian-bom` Bill of Materials is a Maven artifact which defines the versions of dependencies to make dependency management easier.
...
The web archive includes all of the classes in the `inventory` service. If you want to point to a file system to include the classes for your test, you can follow the http://arquillian.org/guides/shrinkwrap_introduction/[Arquillian guide] to do that. For clarity, all of the classes are listed in the deployment block. All of the classes in this service must be added to the web archive, otherwise, the code compiles successfully but fails at runtime when the injection of the testing resource class takes place.

The `testGetPropertiesFromEndpoint` test case is an integration test to test the `inventory` service endpoints. The `@RunAsClient` annotation added in this test case is to indicate that this test case is to be run as a client, which means the tests are run against the managed container. The endpoint test case first calls the `http://localhost:9080/<WARNAME>/inventory/systems/{hostname}` endpoint with the `localhost` hostname, to add its system properties to the inventory. The test verifies that the system property for the local and service JVM should match. Then, the test method calls the `http://localhost:9080/<WARNAME>/inventory/systems` endpoint. The test checks that the inventory should have one host registered and that should be `localhost`. The test also verifies that the system property stored in the inventory for the local and service JVM should match.

Next, refer to the `testInventoryResourceFunctions` test case, which is a functional test case to test the functions in the `InventoryResource` class.

CDI is used to inject an instance of the `InventoryResource` class into this test class. It is then tested by the `testGetPropertiesFromFunction` method, which validates that the system property for the local and service JVM should match. This test case calls the `listContents()` method to get all systems that are stored in this inventory and verifies that one system is found and it is `localhost`. The functional test case does not do the action to store any system in the inventory, the one system that's found was from the endpoint test case that was run prior to this test case. The `@InSequence` annotation from Arquillian's JUnit integration guarantees the test sequence. The sequences are important for the two test methods, as the results in the first test impacts the second one.
...
In the tests output, you see the application server was launched, and that the ShrinkWrap web archive, `arquillian-managed` is started as an application in the server. Then, the tests are being run and you see the test case logs.

----
[AUDIT   ] CWWKE0001I: The server ArquillianManagedServer has been launched.
...
[INFO] Running it.io.openliberty.guides.inventory.InventoryIT
...
[INFO    ] SESN0176I: A new session context will be created for application key default_host/arquillian-managed
[INFO    ] SESN0172I: The session manager is using the Java default SecureRandom implementation for session ID generation.
[AUDIT   ] CWWKZ0001I: Application arquillian-managed started in 0.463 seconds.
...
*****testGetPropertiesFromEndpoint*****
...
Test the endpoint response status code is OK.
Test the system property for the local and service JVM should match.
Test the endpoint response status code is OK.
Test the inventory should have one entry for localhost.
******************************
[INFO    ] SRVE0242I: [arquillian-managed] [/arquillian-managed] [ArquillianServletRunner]: Initialization successful.
*****testInventoryResourceFunctions*****
Test the inventory should have one entry.
Test the inventory should have localhost registered.
Test system property for the local and service JVM should match.
******************************
...
...
[INFO] Tests run: 2, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 2.444 s - in it.io.openliberty.guides.inventory.InventoryIT
...
[INFO] ------------------------------------------------------------------------
[INFO] BUILD SUCCESS
[INFO] ------------------------------------------------------------------------
...
Following the execution of the tests, the test application is automatically destroyed and the server is shut down. You should then get a message indicating that the build and tests are successful.

If you make changes to the test cases, you can verify that the tests pass using the Maven verify goal:
