...
:page-seo-title: Testing microservices with Arquillian Liberty Managed container
:page-seo-description: Learn how to test your microservices with Arquillian Liberty Managed container
...
Arquillian Liberty Managed container with the Maven build tool and develop functional and
...
You will develop tests that use Arquillian and JUnit to verify the `inventory` service as an endpoint
...
----
#Create the `InventoryArquillianTest` test class.#
`src/test/java/it/io/openliberty/guides/inventory/InventoryArquillianTest.java`
----

InventoryArquillianTest.java
[source, Java, linenums, indent=0, role="code_column"]
----
include::finish/src/test/java/it/io/openliberty/guides/inventory/InventoryArquillianTest.java[tags=**;!copyright;]
----
...
The contents of the web archive are defined in this block,
which is going to be deployed to Open Liberty and the tests are run
on or against the server.
The [hotspot=36]`testable = true` attribute enables the deployment to
run in-container tests, which means the tests are run against the server.
...
This annotation provides the hostname, port number and web archive
information for this service, so you do not need to hard code these values
...
the `localhost` hostname, to add its system properties to the inventory.
...
The test checks that the inventory should have one host registered and
...
First, you need the [hotspot=7-11]`liberty-maven-app-parent` block,
which configures the Liberty Maven Plugin with the default executions necessary
for setting up your managed Liberty server.

Next, you need the [hotspot=39-45]`arquillian-bom` Bill of Material.
It is a Maven artifact that defines the versions of Arquillian dependencies to
make dependency management easier.

You also need the [hotspot=115-121]`arquillian-liberty-managed-junit` dependency
bundle and the Arquillian [hotspot=122-126]`shrinkwrap-api`.
The dependency bundle includes all the core dependencies for running Arquillian
tests on a managed Liberty container using JUnit. You can learn more about the
https://github.com/OpenLiberty/arquillian-liberty-dependencies[Arquillian Liberty dependency bundles].
The Shrinkwrap API allows you to create your test archive which is packaged into
a WAR file and deployed to the Open Liberty server.

Finally, you need the [hotspot=140-173]`maven-failsafe-plugin` to run your
Arquillian integration tests using JUnit.

You also need the [hotspot=174-219]`liberty-maven-plugin`
...
values automatically from the Liberty Maven Plugin configuration,
...
The parameter values are needed to run the Arquillian Liberty Managed container tests.
...
[INFO]  T E S T S
[INFO] -------------------------------------------------------
[INFO] Running it.io.openliberty.guides.inventory.InventoryArquillianTest
...
...
technologies that you have come across in this guide.
