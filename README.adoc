...
To try running the finished Arquillian test, navigate to the `start` folder and execute the Maven or Gradle build:
...
You will see the following logs indicating that your test has passed, followed by a message indicating a successful build.
...
Finally, add your test configuration. Notice that the tests depend on the `configureArquillian` task. This task will automatically create and configure your `arquillian.xml` by retrieving the core `wlpHome`, `serverName`, and `httpPort` parameter values automatically from the Liberty Gradle Plugin configuration, and also allows you to specify additional `arquillian.xml` parameters directly in your `build.gradle` file in an `arquillianConfiguration` dictionary. Complete documentation for this goal can be found https://github.com/WASdev/ci.gradle/blob/master/docs/configureArquillian.md[here].
...
Now that you're done configuring the build tool of your choice, it's time to write your test. Notice that the source of the starting project includes two classes, `Greeter` and `PhraseBuilder`. Now you will create a test that uses Arquillian and JUnit to test both of these classes.

Create a test file at `src/test/java/org/arquillian/example/GreeterIT.java`. The core contents of the file are reproduced below. A detailed explanation follows.
...
The first thing you should notice is that JUnit Arquillian runner is being used. This is specified using the `RunWith` annotation at the top of the class, and tells JUnit to run the tests using Arquillian. 

The next important item is the `@Deployment` block. The ShrinkWrap test archive's contents are defined here, which is going to be deployed to Open Liberty to run the in-container tests. Notice the inclusion of the `Greeter` and `PhraseBuilder` classes. Without these, the code will compile successfully but fail at runtime the injection of the `Greeter` class takes place.
