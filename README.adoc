...
Notice the [hotspot=warName hotspot=WebArchive file=0]`WARNAME` war file name that is used for the web
archive and is defined in the [hotspot=ArquillianWarName file=1]`pom.xml` file.
...
When the application runs in the Arquillian Liberty managed container, 
the context root of the application will be assigned as same as the war file name.
Through the [hotspot=arquillianProperties]`arquillianProperties` configuration, 
you can pass the context root information to the application and customize the container.
You can learn more about the `arquillianProperties` configuration in the
https://github.com/OpenLiberty/liberty-arquillian/blob/master/liberty-managed/README.md#configuration[Arquillian Liberty Managed Documentation^].
...
your Arquillian container. You can learn more about the `configure-arquillian` goal in its
